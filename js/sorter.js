function makePacks(setData) {
	...
	var Card = function(colors, colorIdentity, convertedManaCost, frameEffects, manaCost, manaValue, name, number, rarity, types) {
		this.colors = colors;
...
fetch("js/ISD.json", {method: 'get'}).then(response => response.json())
	.then(jsonData => {
		console.log("Hello");
		const data = jsonData;
		makePacks(data);
